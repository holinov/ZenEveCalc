...
<!-- Modified by JAZ, April 12 2010, then again August 2012 -->
...
<!-- Where Calacpercentdone = 100% but task has not been checked off (percentdone < 100%), progess is given as 100%, but the task not deemed to be finished and hence not 'struck through' -->
...
<!--    Table borders are mostly switched off (except bottom border).  
            ...
            Rows have a line above them, adjust td.tborder to change
...
<!--    Text size and colour is changed depending on value and task status.  Feel free to modify. As supplied:
            ...
            Lead / root tasks are bolded - see tr.lead
...
<!--			<xsl:element name="h3">Project:   <xsl:value-of select="@PROJECTNAME"/>  -  <xsl:value-of select="@REPORTDATE"/>
	...
	<xsl:template name="get_LeadTask">
		<xsl:element name="tr">
			...
			<xsl:if test="(@CALCPERCENTDONE=100)">
				<xsl:if test="not(@PERCENTDONE=100)">
					<xsl:attribute name="class">lead</xsl:attribute>
				</xsl:if>
				<xsl:if test="(@PERCENTDONE=100)">
					<xsl:attribute name="class">lead leaddone</xsl:attribute>
				</xsl:if>
			</xsl:if>
	...
	<xsl:template name="get_Task">
		<xsl:element name="tr">
			...
			<xsl:if test="(@CALCPERCENTDONE=100)">
				...
				<xsl:if test="(@PERCENTDONE=100)">
					<xsl:attribute name="class">task taskdone</xsl:attribute>
				</xsl:if>
			</xsl:if>
			<xsl:call-template name="get_Task_Details"/>
		</xsl:element>
		...
		<xsl:element name="td">
			...
			<xsl:if test="count(ancestor::TASK)>0">			
				<xsl:if test="($lineforTask=1)">
					<xsl:attribute name="class">tborder</xsl:attribute>
				</xsl:if> 
			</xsl:if> 
                       <xsl:if test="count(ancestor::TASK)=0">
				...
				<xsl:if test="($showbullet=1)">
					<xsl:text>&#149; </xsl:text><!--This doesn't seem to work - puts in a square (character not found)-->
				</xsl:if>				
			</xsl:if>
			...
			<xsl:text>&#160; &#160;</xsl:text> <!-- &nbsp;-->
                       ...
                       <!--Addition of additional information to the task title-->
			<xsl:element name="span">
				...
				<xsl:if test="($showpeople=1) and @PERSON">
					...
					<xsl:text>]</xsl:text>
                       ...
                       <!--Addition of comments to the task title (in a box below the title)-->
	...
	<!-- 70 - reformat the date into dd mmm yy format-->
	    ...
	    <!--can't seem to get the 'year-from-date()' functions to work :-(-->
...
<!-- Ideally this following code should work instead of the string manipulation code below - but it doesn't seem to-->

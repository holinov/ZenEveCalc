...
body {
	font-family: Arial;
		...
		<xsl:element name="body">
			<xsl:element name="h1">Report: <xsl:value-of select="@REPORTTITLE"/>
			</xsl:element>
	...
	<xsl:template name="get_Task_Details">
		...
		<xsl:call-template name="get_LastMod"/>
	...
	<xsl:template name="get_LastMod">
		<xsl:element name="td">
			<xsl:choose>
				<xsl:when test="(@LASTMODSTRING)">
					<xsl:attribute name="class">LastMod bbasic</xsl:attribute>
					<xsl:value-of select="@LASTMODSTRING"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="class">LastMod bbasic empty</xsl:attribute>
					<xsl:text>non</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:element>

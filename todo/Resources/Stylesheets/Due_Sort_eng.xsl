...
<!-- This file displays 
        ALL         not-done tasks
        SORTED      by due-date
        WITH        "Due date / Allocated to / Task name"    -->
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="TODOLIST">
        <html>
        <head>
                <title>Project Report 'Due-Dates': <xsl:value-of select="@PROJECTNAME" /></title>
            <xsl:call-template name="style"/>
        </head>
        <body>
        <xsl:element name="h2">Project Report 'Due-Dates': <xsl:value-of select="@PROJECTNAME"/></xsl:element>

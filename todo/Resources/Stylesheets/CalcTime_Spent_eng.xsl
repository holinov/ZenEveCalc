...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!--<xsl:param name="sortby" select="'CALCTIMESPENT'" />-->
    ...
    <xsl:template match="TODOLIST">
        <html>
        <head>
                <title>Project Report 'Calculated Spent-Times': <xsl:value-of select="@PROJECTNAME" /></title>
            <xsl:call-template name="style"/>
        </head>
        <body>
        <xsl:element name="h2">Project Report 'Calculated Spent-Times': <xsl:value-of select="@PROJECTNAME"/></xsl:element>
        ...
        <h5>Attention: Each task contains the cumulated time of all sub-tasks! Do NOT cumulate these values again!</h5>

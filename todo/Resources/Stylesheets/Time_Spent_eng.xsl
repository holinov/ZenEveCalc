...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="TODOLIST">
        <html>
        <head>
                <title>Project Report 'Spent-Times': <xsl:value-of select="@PROJECTNAME" /></title>
            <xsl:call-template name="style"/>
        </head>
        <body>
        <xsl:element name="h2">Project Report 'Spent-Times': <xsl:value-of select="@PROJECTNAME"/></xsl:element>
        ...
        <h5>Attention: Values per Task and cumulated values are displayed together!</h5>
